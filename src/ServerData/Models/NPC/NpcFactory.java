        ...
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                new Thread(() -> {
                    try {
                        while (true) {
                            String[] textLucy = {"|7|KAMUI", "|7|Chào mừng đến với NRO KAMUi !",
                                "|7|KAMUI!"};
                            Thread.sleep(5000);
                            new Thread(() -> {
                                try {
                            ...
                            case 0:
                                if (player.nPoint.limitPower < NPoint.MAX_LIMIT) {
                                    this.createOtherMenu(player, ConstNpc.OPEN_POWER_MYSEFT,
                                            ...
                                            "Nâng\ngiới hạn\nsức mạnh",
                                            "Nâng ngay\n" + Util.numberToMoney(OpenPowerService.COST_SPEED_OPEN_LIMIT_POWER) + " ngọc xanh", "Đóng");
                                } else {
                                    this.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                        ...
                        switch (select) {
                            ...
                            case 1:
                                if (player.inventory.gem >= OpenPowerService.COST_SPEED_OPEN_LIMIT_POWER) {
                                    ...
                                    Service.gI().sendThongBao(player,
                                            "Bạn không đủ ngọc xanh để mở, còn thiếu "
                                            + Util.numberToMoney((OpenPowerService.COST_SPEED_OPEN_LIMIT_POWER - player.inventory.gem)) + " ngọc xanh");
                                }
                                break;
                        }
                        ...
                        if (player.iDMark.getIndexMenu() == ConstNpc.BANKING) {
                            switch (select) {
                                ...
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    NpcService.gI().createMenuConMeo(player, ConstNpc.DOI_SKILL, 12639, "|7|DragonBall DEV" + "\n"
                                            + "ĐỔI SKILL ĐỆ TỬ\n"
                                            + "|2|Chào bạn : " + player.name
                                            + "\nSố Dư : " + Util.format(player.getSession().vnd) + " VNĐ\n"
...
//                                Service.gI().sendThongBao(player, "Đã " + (TaiXiu_Old.gI().baotri == true ? "Tắt" : "Bật") + " Tài Xỉu!");
                            ...
                            case 11:
                                ...
                                do{
                                    i++;
                                    Service.gI().addSMTN(player, (byte) 2, 2123456789, false);
                                    if(player.pet != null){
                                        Service.gI().addSMTN(player.pet, (byte) 2, 2123456789, false);
                                    }
                                    try {
                                        Thread.sleep(1000);

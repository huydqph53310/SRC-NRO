...
public class SummonDragon {
    ...
    private final int timeResummonShenron = 360000;
    ...
    public void summonNamec(Player pl) {
        ...
        for (Player player : pl.zone.getPlayers()) {
            if (!player.equals(pl) && player.clan != null) {
                nPlSameClan++;
            }
            if (nPlSameClan == pl.clan.membersInGame.size()) {   
                if (pl.zone.map.mapId == 7 && pl.clan != null) {
                ...
                Service.gI().sendThongBao(pl, "Hãy hợp đủ thành viên bang đang online");
            }
        }
    }
    ...
    private void sendWhishesNamec(Player pl) {
        NpcService.gI().createMenuRongThieng(pl, ConstNpc.NAMEC_1, "Ta sẽ ban cho cả bang ngươi 1 điều ước, ngươi có 5 phút, hãy suy nghĩ thật kỹ trước khi quyết định"
                + "(Chú ý: Phần thưởng sẽ được chia đều cho toàn bộ thành viên)", "x99 ngọc rồng 3 sao");
    }
    ...
    public void confirmWish() {
        switch (this.menuShenron) {
                ...
                switch (this.select) {
                    case 0: //đẹp trai nhất vũ trụ
                        if (InventoryServiceNew.gI().getCountEmptyBag(playerSummonShenron) > 0) {
                            ...
                            avtVip.itemOptions.add(new ItemOption(50, Util.nextInt(10, 25)));
                            avtVip.itemOptions.add(new ItemOption(77, Util.nextInt(10, 25)));
                            avtVip.itemOptions.add(new ItemOption(103, Util.nextInt(10, 25)));
            ...
            case ConstNpc.SHENRON_2:
                switch (this.select) {
                    case 0: //+214 hồng ngọc
                        this.playerSummonShenron.inventory.ruby += 214;
                        PlayerService.gI().sendInfoHpMpMoney(this.playerSummonShenron);
                        break;
                    case 1: //+20 tr smtn
                        if (this.playerSummonShenron.nPoint.power >= 20000000) {
                            Service.gI().addSMTN(this.playerSummonShenron, (byte) 0, 20000000, false);
                        } else {
                            Service.gI().sendThongBao(this.playerSummonShenron, "Cần 20tr sức mạnh");
                        } 
                        break;
                    case 2: //2 tr vàng
            ...
            case ConstNpc.SHENRON_3:
                switch (this.select) {
                    case 0: //+30 hồng ngọc
                        this.playerSummonShenron.inventory.gem += 30;
                        PlayerService.gI().sendInfoHpMpMoney(this.playerSummonShenron);
                        break;
                    case 1: //+2 tr smtn
                        if (this.playerSummonShenron.nPoint.power >= 2000000) {
                            Service.gI().addSMTN(this.playerSummonShenron, (byte) 0, 2000000, false);
                        } else {
                            Service.gI().sendThongBao(this.playerSummonShenron, "Cần 2tr sức mạnh");
                        } 
                        break;
                    case 2: //200k vàng
                    ...
                    if(playerSummonShenron.clan != null){
                        playerSummonShenron.clan.members.forEach(m->{
                            if(Client.gI().getPlayer(m.id) != null){
                                ...
                                it.quantity = 99/playerSummonShenron.clan.membersInGame.size();

        ...
        if (pl != null) {
            int sl = InventoryServiceNew.gI().findItemBag(pl, (short)457) == null ? 0 : InventoryServiceNew.gI().findItemBag(pl, (short)457).quantity;
                        NpcService.gI().createMenuConMeo(player, ConstNpc.QUANLYTK, 12639, "|7|[ MANAGER ACCOUNT ]"
                                +"\n|5|Player : " + pl.name + (pl.vip > 0 && pl.vip < 4 ? " [VIP" + pl.vip + "]" : pl.vip == 4 ? " [SVIP]" : "")
                                +"\nAccount ID : " + pl.id + " | "+ "IP Connect : " + pl.getSession().ipAddress +" | "  + "Version Mod : " + pl.getSession().version
...
//            e.printStackTrace();
    ...
    public void regisAccount(Session session, Message _msg) {
        try {
            ...
            GirlkunResultSet checklog = GirlkunDB.executeQuery("select * from anti_regis where ip_session = ?",session.getIP());
            if (checklog.first()) {
            int count = checklog.getInt("count");
                    sendThongBaoOK((MySession) session, "Vui lòng đợi...\nĐang tiến hành kiểm tra dữ liệu!");
                    Thread.sleep(3000);
                if (count >= 3000) {
                    sendThongBaoOK((MySession) session, "Đăng ký tài khoản không thành công!\nSố lượng tài khoản đạt đến giới hạn cho phép!");
                    checklog.dispose();
                    return;
                }
            } else {
            ...
            if (rs.first()) {
                ...
                Thread.sleep(3000);
                GirlkunDB.executeUpdate("update anti_regis set count = (count + ?) where ip_session = ?", 1,session.getIP());
                GirlkunDB.executeUpdate("insert into account (username, password, ip_address, active) values()", user, pass,session.getIP(),1);
...
// Lệnh ADmin
        ...
        if (player.getSession() != null && player.isAdmin2()||player.getSession() != null && player.isAdmin()) {
            ...
            List<Skill> skfix2 = new ArrayList<>();
                ...
                try {
                    ...
                    if (BossManager.gI().getBossById(Integer.parseInt(getID)) != null && !BossManager.gI().getBossById(Integer.parseInt(getID)).isDie()) {
                ...
                try {
                    ...
                    if (b != null) {
                        ...
                        NpcService.gI().createMenuConMeo(player, ConstNpc.QUANLYBOSS, 12639, "|7|[ BOSS MANAGER ]"
                                +"\n|2|Boss : " + b.name + "(" + id + ")"
                                +"\nTrạng thái : " + (b.typePk == 0 && b.zone != null ? "nonActive " : b.typePk == 5 && b.zone != null ? "Actived " : "") + (b.zone != null ? "(Sống)" : "(Chết)")
                                +(b.zone != null ? "\nHP: "+Util.powerToString(b.nPoint.hp) 
                                +"\nDame: "+Util.powerToString(b.nPoint.dame)
                                +"\nMap : "+b.zone.map.mapName + "(" + b.zone.map.mapId + ")" +"\nZone: " + b.zone.zoneId : "")
                                +"\n|7|[ DRAGONBALL XUANLAM]","CREATE","REMOVE", "KILL","ACTIVE","RESPAWN");
                    } else {
                        Service.gI().sendThongBao(player, "Boss is Die!");
                        return;
                    }
                ...
                try {
                    ...
                    if (BossManager.gI().bosses.size() < 137) {
                        Service.gI().sendThongBaoFromAdmin(player, "Clean Success!\n|7|Boss size : (" + BossManager.gI().bosses.size() + ")");
                        break;
                    }
                    for (int i = 136; i < BossManager.gI().bosses.size(); i++) {
                        Boss b = BossManager.gI().getBossById(i);
                        if (b != null) {
                        if (MapService.gI().isMapMaBu(b.data[0].getMapJoin()[0]) || MapService.gI().isMapBlackBallWar(b.data[0].getMapJoin()[0]) || MapService.gI().isMapDoanhTrai(b.data[0].getMapJoin()[0])) {
                        continue;
                        }
                        b.die(null);
                ...
                try {
                    ...
                    if (Client.gI().getPlayer(String.valueOf(getID)) != null && !Client.gI().getPlayer(String.valueOf(getID)).isDie()) {
                        Client.gI().getPlayer(String.valueOf(getID)).setDie(Client.gI().getPlayer(String.valueOf(getID)));
    ...
    try {
        ...
        if (player.getSession() != null && player.isAdmin2()) {
                ...
                return;
                    ...
                    return;
                } else if (text.equals("buffitem")) {
                    Input.gI().createFormGiveItem(player);
                    return;
                }else if (text.equals("an")){//???
                Input.gI().createFormGiveItem(player);
        ...
        if (text.equals("fullskill")) {
            List<Skill> skfix = new ArrayList<>();
            if(player.gender == 0){ // td
                skfix.add(SkillUtil.createSkill(0,1));
                skfix.add(SkillUtil.createSkill(1,7));
                skfix.add(SkillUtil.createSkill(6,7));
                skfix.add(SkillUtil.createSkill(9,7));
                skfix.add(SkillUtil.createSkill(10,7));
                skfix.add(SkillUtil.createSkill(20,7));
                skfix.add(SkillUtil.createSkill(22,7));
                skfix.add(SkillUtil.createSkill(19,7));
                skfix.add(SkillUtil.createSkill(24,7));
            ...
            if(player.gender == 1){ // namek 2, 3, 7, 11, 12, 17, 18,26, 19}
                skfix.add(SkillUtil.createSkill(2,7));
                skfix.add(SkillUtil.createSkill(3,7));
                skfix.add(SkillUtil.createSkill(7,7));
                skfix.add(SkillUtil.createSkill(11,7));
                skfix.add(SkillUtil.createSkill(12,7));
                skfix.add(SkillUtil.createSkill(17,7));
                skfix.add(SkillUtil.createSkill(18,7));
                skfix.add(SkillUtil.createSkill(19,7));
                skfix.add(SkillUtil.createSkill(26,7));
                player.playerSkill.skills = skfix;
                sendThongBao(player, "Fix Skill Thành Công, Hãy Học Lại Skill");
                return;
            }
            if(player.gender == 2){ // xd 4, 5, 8, 13, 14, 21, 23, 25, 19}
                skfix.add(SkillUtil.createSkill(4,7));
                skfix.add(SkillUtil.createSkill(5,7));
                skfix.add(SkillUtil.createSkill(8,7));
                skfix.add(SkillUtil.createSkill(13,7));
                skfix.add(SkillUtil.createSkill(14,1));
                skfix.add(SkillUtil.createSkill(21,7));
                skfix.add(SkillUtil.createSkill(23,7));
                skfix.add(SkillUtil.createSkill(19,7));
                skfix.add(SkillUtil.createSkill(25,7));
                player.playerSkill.skills = skfix;
                sendThongBao(player, "Fix Skill Thành Công, Hãy Học Lại Skill");
                return;
            }
            ...
            NpcService.gI().createMenuConMeo(player, ConstNpc.INFO, 12639,"|7|- •⊹٭DragonBall NRO_XUANLAM⊹• -\n"
            ...
            switch (text) {
                ...
                case "fusion":
                    ...
                    if (btc1.isNotNullItem())UseItem.gI().usePorata(player);
                    if (btc2.isNotNullItem())UseItem.gI().usePorata2(player);
                    if (btc3.isNotNullItem())UseItem.gI().usePorata3(player);
                    return;
                }
            }
        if (text.length() > 100) {
    ...
    public void menuAd(Player player) {
        ...
        if (player.getSession() != null && player.isAdmin2()) {
            NpcService.gI().createMenuConMeo(player, ConstNpc.QUANTRI1, 12639, "|7|[ - Admin : " + player.name +" - ]\n"
            ...
            +"\n|7|CPU : " + cpuUsage + "/100%"
            +"\nRAM : " + usedPhysicalMemoryStr + "/16 GB"
...
//            Send_Info_NV(pl);
    ...
    public void sendTitleRv(Player player, Player p2, String tendanhhieu) {
        ...
        try {
            ...
            if (player.titlett == true) {
                switch (tendanhhieu) {
                    case "VECHAI": me.writer().writeShort(84);break;
                    case "FANCUNG": me.writer().writeShort(85);break;
                    case "SUCMANH": me.writer().writeShort((player.gender == 0 ? 223 : player.gender == 1 ? 222 : 224));break;
                    case "PHONGBA": me.writer().writeShort(220);break;
                    case "TUOITHO": me.writer().writeShort(65);break;
                }
            }
            if (player.titleitem == true) {
                switch (tendanhhieu) {
                    case "DH1": if (player.timedh1 > 0 && player.usedh1) {me.writer().writeShort(67);}break;
                    case "DH2": if (player.timedh2 > 0 && player.usedh2) {me.writer().writeShort(73);}break;
                    case "DH3": if (player.timedh3 > 0 && player.usedh3) {me.writer().writeShort(64);}break;
                    case "VIP1": me.writer().writeShort(60);break;
                    case "VIP2": me.writer().writeShort(61);break;
                    case "VIP3": me.writer().writeShort(62);break;
                    case "VIP4": me.writer().writeShort(63);break;
                }
            }}
    ...
    public void sendTitle(Player player, String tendanhhieu) {
        Message me;
        try {
            me = new Message(-128); me.writer().writeByte(0); me.writer().writeInt((int) player.id);
            if (player.titlett == true) {
                switch (tendanhhieu) {
                    case "VECHAI": me.writer().writeShort(84);break;
                    case "FANCUNG": me.writer().writeShort(85);break;
            ...
            me.writer().writeByte(1); me.writer().writeByte(-1); me.writer().writeShort(-1); me.writer().writeByte(-1); me.writer().writeByte(-1);
            this.sendMessAllPlayerInMap(player, me); me.cleanup();
        } catch (IOException e) {
        }
    }
    ...
    public void reloadTitle(Player player) {
            ...
            if (player.inventory.itemsBody.size() > 12 && player.inventory.itemsBody.get(12).isNotNullItem() && player.inventory.itemsBody.get(12) != null) {
                Service.getInstance().addEffectChar(player, (player.inventory.itemsBody.get(12).template.part), 0, -1, -1, 1);
            }
...
//             if (player.pet.inventory.itemsBody.get(8).isNotNullItem() && player.pet.inventory.itemsBody.get(8) != null && player.pet.reloadtitle == true) {
//                Service.getInstance().sendPetFollow(player.pet, (short) ((short) player.pet.inventory.itemsBody.get(8).template.iconID - 1));
//            }
...
//            }            
    ...
    public void sendFlagBag(Player pl) {
        ...
        try {
            ...
            if (pl.reloadtitle == true) flagbag = -1;
            if (pl.isPl() && pl.getSession().version > 228){
                if(pl.fusion.typeFusion==ConstPlayer.HOP_THE_GOGETA)flagbag = 204;
                switch (flagbag) {
                    case 108:
                    flagbag = 204;
                    break;

...
import ServerData.Boss.ListBoss.FideGold;
...
import ServerData.Boss.ListBoss.BROLY.BrolySuper;
import ServerData.Boss.ListBoss.BROLY.SuperBroly;
...
public class BossManager implements Runnable {
    ...
    public void loadBoss() {
        ...
        try {
            ...
            for (int i = 1; i <= 5; i++) {
                this.createBoss(BossID.BROLY_SUPER + i);
            }
        } catch (Exception ex) {
        }
    ...
    public Boss createBoss(int bossID) {
        try {
            if (bossID <= BossID.BROLY_THUONG + 10 && bossID >= BossID.BROLY_THUONG) {

                return new Broly(MapService.gI().getZone(Util.randomMapBossBroly()), Util.nextInt(10_000, 200_070), 20000);
            }
            if (bossID <= BossID.BROLY_SUPER + 10 && bossID >= BossID.BROLY_SUPER) {

                return new BrolySuper(MapService.gI().getZone(Util.randomMapBossBroly()), Util.nextInt(10_000, 200_070), 200000);
            }
            switch (bossID) {
